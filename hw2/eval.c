...
Value eval(Exp e, Valenv globals, Funenv functions, Valenv formals) {
    ...
    case VAR:     /*
                   ...
                   * which is implemented in C by calling \monobox
                   * isvalbound(e->var, formals). If x \notindom rho and x
                   * \notindom xi, the operational semantics gets stuck—so
                   * the interpreter issues an error message. Less
                   * formally, we look up the variable by checking first
                   * the local environment and then the global
                   * environment.
    ...
    case SET:     /*
                   ...
                   * domain of rho ([[formals]]). \ptrim-4 Because both
                   * rules require the premise \evale ==>\eval[']v, we
                   * evaluate the right-hand side first and put the result
                   * in [[v]].
    ...
    case IFX:     /*
                   ...
                   * [']v_1. To get v_1, xi', and rho', we call \monobox
                   * eval(e->ifx.cond, globals, functions, formals)
                   * recursively. This call is safe only because the new
                   * environments xi' and rho' are used in the third
                   * premises of both rules. If xi' and rho' were not
                   * always used, we would have had to make copies and
                   ...
                   * expression e_2 is [[e->ifx.truex]]; e_3 is [[e->
                   * ifx.falsex]]. [*]
    ...
    case WHILEX:  /*
                   * There are also two rules for while loops. A
                   * systematic translation of \eval['']\while(e_1, e_2) =
                   * =>\eval[''']v_3, where it appears as a premise of the
                   * first rule, would produce a recursive call to [[eval
                   * (e, ...)]]. Because we know [[e]] is a while loop,
                   ...
                   * valuable because during the execution of a long while
                   * loop in Impcore, it can keep the C stack from
                   * overflowing.
    ...
    case APPLY:   /*
                   ...
                   * of phi(f). In the code, f is [[e->apply.name]].
                   ...
                   */
                  {
                      ...
                      switch (f.alt) {
                      case USERDEF:   /*
                                       * The premises of the \rulenameApplyUser
                                                                    rule require
                                       * that the list of formal parameters to f
                                                                     be the same
                                       * length as the list of actual parameters
                                                                    in the call.
                                       * I let [[xs]] represent the formals \
                                                                     ldotsnx and
                                       * [[vs]] represent the actuals v_1, ...,
                                                                     v_m. If the
                                       * formals and actuals are the same length
                                                                       , so m=n,
                                       * I use \monoboxmkValenv(xs, vs) to
                                                                  create a fresh
                                       * environment \nomathbreak{x_1 |->v_1, ..
                                                                   ., x_n |->v_n
                      ...
                      case PRIMITIVE: /*
                                       * Impcore has few primitive operators,
                                                                    and they are
                                       * simple. We handle [[print]] separately
                                                                        from the
                                       * arithmetic primitives. More general
                                                                  techniques for
                                       * implementing primitives, which are
                                                                 appropriate for
                                       * larger languages, are shown as part of
                                                                             the
                                       * implementation of micro-Scheme in \
                                                                        crefpage
                                    ...
                                    apply.actuals, globals, functions, formals);
                                          if (f.primitive == strtoname("print"))
                                              /*
                                               * Primitive [[print]] is shown
                                                           here; [[println]] and
                                               * [[printu]] appear in \crefapp:
                                                                        impcore.
                                              ...
                                              /*
                                               * Because the name of each
                                                        arithmetic primitive has
                                               * just one character, I switch on
                                                            that character. This
                                               * technique is reasonable only
                                                              because the set of
                                               * primitives is small and fixed.
                                               ...
                                               */
                                              {
                                                  ...
                                                  /*
                                                   * <check that [[vs]] has
                        exactly two values, and assign them to [[v]] and [[w]]>=
                                                   */
                                ...
                                overflow on [[v]] and [[w]], call [[runerror]]>=
                                              ...
                                              /*
                                               * The code also depends on the
                                                        fact that Impcore shares
                                               * C's rules for the values of
                                                         comparison expressions.
                                               * Impcore does not share C's
                                                      other rules of arithmetic:
                                               * in Impcore, if the result of an
                                                            arithmetic operation
                                               * does not fit in the range -2^31
                                                                    to 2^31, the
                                               * operation causes a checked run-
                                                            time error. Function
                                               * [[checkarith]] is defined in \
                                                               crefapp:cinterps.
                                               */
...
/*
 * The assertion at the end of [[eval]] might seem
 * superfluous, but it isn't; it helps protect me,
 * and you, from mistakes. If I forget a case in the
 * [[switch]], the evaluator will halt with an error
 * message, rather than silently do something
 * unpredictable. Another way to protect yourself from
 * mistakes is to turn on all compiler warnings, which
 * I do; calling [[assert(0)]], which the compiler knows
 * can never return, keeps the C compiler from issuing a
 * warning that [[eval]] might not return.
 */
 ...
 * order of evaluation, we call [[eval(es->hd,]] ...
 * [[)]] and [[evallist(es->tl,]] ...[[)]] in separate
 * C statements. Writing both calls as parameters to
 * [[mkVL]] would be a mistake because C makes no
 * guarantees about the order in which the actual
 * parameters of a function are evaluated. [*]
 */
...
/*
 * Responsibility for evaluating definitions is shared
